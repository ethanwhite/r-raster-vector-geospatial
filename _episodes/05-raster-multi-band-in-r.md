...
> >  [17] Arith          as.array       as.data.frame  as.integer    
> >  [21] as.list        as.logical     as.matrix      as.vector     
> >  [25] atan2          bbox           boxplot        brick         
> >  [29] calc           cellStats      clamp          click         
> >  [33] coerce         colSums        Compare        coordinates   
> >  [37] corLocal       cover          crop           crosstab      
> >  [41] cut            cv             density        dim           
> >  [45] dim<-          disaggregate   dropLayer      extend        
> >  [49] extent         extract        flip           freq          
> >  [53] getValues      getValuesBlock getValuesFocal head          
> >  [57] hist           image          interpolate    intersect     
> >  [61] is.factor      is.finite      is.infinite    is.na         
> >  [65] is.nan         isLonLat       KML            labels        
> >  [69] length         levels         levels<-       log           
> >  [73] Logic          mask           match          Math          
> >  [77] Math2          maxValue       mean           merge         
> >  [81] minValue       modal          mosaic         names         
> >  [85] names<-        ncell          ncol           nlayers       
> >  [89] nrow           origin         origin<-       overlay       
> >  [93] pairs          persp          plot           plotRGB       
> >  [97] predict        print          proj4string    proj4string<- 
> > [101] quantile       raster         rasterize      readAll       
> > [105] readStart      readStop       reclassify     res           
> > [109] resample       rotate         rowSums        sampleRandom  
> > [113] sampleRegular  scale          select         setMinMax     
> > [117] setValues      shift          show           spplot        
> > [121] stack          stackSelect    subs           subset        
> > [125] Summary        summary        t              tail          
> > [129] text           trim           unique         unstack       
> > [133] values         values<-       weighted.mean  which.max     
> > [137] which.min      whiches.max    whiches.min    writeRaster   
> > [141] xmax           xmin           xres           ymax          
> > [145] ymin           yres           zonal          zoom          
...
> > 
> > ~~~
> >  [1] [             [<-           anyDuplicated as_data_frame as_tibble    
> >  [6] as.data.frame as.raster     as.tbl_cube   bind          boxplot      
> > [11] brick         coerce        coordinates   determinant   duplicated   
> > [16] edit          extent        extract       head          initialize   
> > [21] isSymmetric   Math          Math2         Ops           raster       
> > [26] rasterize     relist        subset        summary       surfaceArea  
> > [31] tail          trim          unique        weighted.mean writeValues  
> > see '?methods' for accessing help and source code
> > ~~~
> > {: .output}
